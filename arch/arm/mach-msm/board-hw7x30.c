...
/* add 1M ,because LCD use three buffers and tvout use one buffer */
...
static uint32_t camera_off_gpio_table[] = {
    ...
    GPIO_CFG(16, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* SCL */
	GPIO_CFG(17, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* SDL */
...
#ifdef CONFIG_HUAWEI_KERNEL
void msm_snddev_poweramp_4qchat_on(void)
{
	gpio_set_value(82, 1);	/* enable spkr poweramp */
    if(audio_amplifier_data.amplifier_4qchat_on)
        audio_amplifier_data.amplifier_4qchat_on();
    if(right_audio_amplifier_data.amplifier_4qchat_on)
		right_audio_amplifier_data.amplifier_4qchat_on();
	pr_info("%s: power on amplifier for qchat\n", __func__);
...
{
    gpio_set_value(33, 1);	/* enable hac parallel inductance */
    pr_info("%s: enable hac gpio\n", __func__);
}

void msm_snddev_hac_off(void)
{
    gpio_set_value(33, 0);	/* disable hac parallel inductance */
    pr_info("%s: disable hac gpio\n", __func__);
}
#endif
...
/*add by xuhui for RTS .5V?? test +  */
/*
    printk(KERN_ERR "--------------reconfig cts, rts by xuhui\n");
			rc = msm_gpios_request_enable(bt_config_bcm4329_power_on,
			ARRAY_SIZE(bt_config_bcm4329_power_on));


if (rc < 0)
printk(KERN_ERR "--------------reconfig cts, rts failed by xuhui\n");
*/
/*add by xuhui for RTS .5V?? test - */
...
static struct msm_gpio wifi_config_power_off[] = {
    { GPIO_CFG(WLAN_REG, WLAN_GPIO_FUNC_0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
        "WL_REG_ON" }, 
    { GPIO_CFG(WLAN_PWR, WLAN_GPIO_FUNC_0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
        "WL_PWR_ON" }
};
*/	   
...
/*	rc = gpio_tlmm_config(wlan_wakes_msm[0], GPIO_CFG_ENABLE);	
	if (rc)		
		printk(KERN_ERR "%s: Failed to configure GPIO[128] = %d\n",__func__, rc);
*/
