...
//slot 3 is used for wifi on 7230
...
/*
			...
			if (mmc->card && mmc->card->type == MMC_TYPE_SDIO &&
					!host->plat->sdiowakeup_irq && (SDCC_WIFI_SLOT != host->pdev_id)) {
#else
*/
			if (mmc->card && mmc->card->type == MMC_TYPE_SDIO &&
...
static void msmsdcc_req_tout_timer_hdlr(unsigned long data)
{
	...
	if (cmd) {
		...
		printk("host->mmc->ios->clock=%u, host->sdcc_suspending=%u, host->clk_rate=%u\n", 
			host->mmc->ios.clock, host->sdcc_suspending, host->clk_rate);
