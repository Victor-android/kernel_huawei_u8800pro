...
void mdp4_overlay_update_lcd(struct msm_fb_data_type *mfd)
{
	...
	if (mddi_pipe == NULL) {
		...
		mddi_ld_param = 0;
		mddi_vdo_packet_reg = mfd->panel_info.mddi.vdopkt;
		/* set the lcd frame frequency is 60HZ  */
/* disable the config MDP reg,resolve freeze screen when turn off camera */
		if (mdp_hw_revision == MDP4_REVISION_V2_1) {
			uint32	data;
				...
				struct mdp4_overlay_pipe *pipe)
{
    ...
    /*for resolving freeze screen because of 60 frame freq*/
    /* disable the config MDP reg,resolve freeze screen when enter camera */
	if (overlay_set)
	{
		if (MDP4_REVISION_V2_1 == mdp_hw_revision)
		{
			uint32	data;
			data = inpdw(MDP_BASE + 0x0028);
			data &= ~0x0300;	/* bit 8, 9, MASTER4 */
    ...
    /* set the config MDP reg,set frame freq 60 hz */
	if (overlay_unset)
	{
		if (MDP4_REVISION_V2_1 == mdp_hw_revision)
		{
