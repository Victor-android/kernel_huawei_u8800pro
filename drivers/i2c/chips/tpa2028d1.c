...
static char en_data_4qchat_u8820[] = 
{
    /* reg  val  */
    0x01, 0x83,
    0x02, 0x03,
...
static char* pen_data_4qchat = &(en_data_4qchat[0]);
static int     pen_data_4qchat_size = 0;
...
static void tpa2028d1_amplifier_4qchat_on(void)
{
    int ret = 0;

    /* power on tpa2028d1 amplifier by type */
    if (0 == pen_data_4qchat_size)
    {
        TPA_DEBUG_TPA("failed to turn on tpa2028d1_amplifier_4qchat, pen_data_4qchat_size = 0.\n");
        return;
    }

    ret = tpa2028d1_amplifier_on_by_type(pen_data_4qchat, pen_data_4qchat_size);
    if(ret)
        TPA_DEBUG_TPA("failed to turn on tpa2028d1_amplifier\n");
    else
        TPA_DEBUG_TPA("tpa2028d1_amplifier_4qchat_on\n");
...
static int tpa2028d1_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    else if (machine_is_msm7x30_u8820())
    {
        ...
        pen_data_4qchat = &(en_data_4qchat_u8820[0]);
        pen_data_4qchat_size = ARRAY_SIZE(en_data_4qchat_u8820);
    ...
    else
    {
        ...
        pen_data_4qchat = &(en_data_4qchat[0]);
        pen_data_4qchat_size = ARRAY_SIZE(en_data_4qchat);
    ...
    if(!ret && (REG1_DEFAULT_VALUE == r_data) && pdata)
    {
        ...
        pdata->amplifier_4qchat_on = tpa2028d1_amplifier_4qchat_on;
        #endif
    }
    return ret;
}
...
static int tpa2028d1_remove(struct i2c_client *client)
{
    ...
    if(pdata)
    {
        ...
        pdata->amplifier_4music_on = NULL;
        pdata->amplifier_4qchat_on = NULL;        
    }
	return 0;
}
