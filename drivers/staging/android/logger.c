			 ...
			 unsigned long nr_segs, loff_t ppos)
{
	...
	for (curr_buf = temp_buf, i = 0, curr_logbytes = total_logbytes;
		...
		/*0 means priority,1 means tags,2 means message*/
        /*this is msg*/
		if(i == 2)
		{
			/*we save the length of the log buffer,if it is not longer than the length of the message*/
			msg_len = len;
		}
		if (!len)
			break;
