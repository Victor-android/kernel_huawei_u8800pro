...
static int serial_str_id=-1;//string id of sequence number in string descriptor
...
void set_usb_sn(char *sn_ptr)
{
    ...
    else
    {
        ...
        serial_number[len] = '\0';
        usb_sn_valid = 1;
        /* if sn_ptr isn't null and id is valid, set the iSerialNumber to serial_str_id */
        if (-1 != serial_str_id) /* -1 is serial_str_id's initialization value */
        {
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	__func__, f->name, enable, dev->adb_enable, f->hidden);
