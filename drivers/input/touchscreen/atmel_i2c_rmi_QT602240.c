...
/* changge the rang of keys */
#define TS_KEY_Y_MAX 73//248
...
/* the value 24 (the gap between touch region and key region)maybe need to modify*/
	  /* changge the rang of keys */
      {(TS_X_MAX*1/8),   (TS_Y_MAX-TS_KEY_Y_MAX/2+15), TS_X_MAX/10, TS_KEY_Y_MAX/3, KEY_BACK},  /*back key */
       {(TS_X_MAX*3/8),   (TS_Y_MAX-TS_KEY_Y_MAX/2+15), TS_X_MAX/10, TS_KEY_Y_MAX/3, KEY_MENU},  /* menu key */
       {(TS_X_MAX*5/8),   (TS_Y_MAX-TS_KEY_Y_MAX/2+15), TS_X_MAX/10, TS_KEY_Y_MAX/3, KEY_HOME},  /* KEY_F2,KEY_HOME home key */
       {(TS_X_MAX*7/8),   (TS_Y_MAX-TS_KEY_Y_MAX/2+15), TS_X_MAX/10, TS_KEY_Y_MAX/3, KEY_SEARCH},  /* Search key */
    },
};
...
/* ACQUISITION processor address. */
u16 acquisition_config_address;
...
#endif
    /* add some variables */
    /* the flag of the first press */
	uint8_t finger_pressed;
    /* the number of finger on the screen */
	uint8_t finger_count;
    /* the flag of recalibration */
	uint8_t Recal_flag;
    /* the time of resume */
	uint64_t timestamp;
    /* record the current coordinate */
	struct atmel_finger_data finger_data[2];
    /* record the first press coordinate */
	struct atmel_finger_data pre_data[2];
...
uint8_t calibrate_chip_error(void)
{
	...
	if(0 == ret)
	{
        ...
        /* delete some lines which is not needed */
	}
    else
    {
...
void check_chip_calibration(void)
{
	...
	if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00))
	{
        ...
        /* check error */
        /* change the value of error */
		if(((tch_ch + atch_ch) > 5)  && (atch_ch >2))
		{					
...
/* delete some lines which is not needed */
/* delete some lines which is not needed */
/* add a function that confirm calibration */
static void confirm_calibration(struct atmel_ts_data *ts)
{
	int ret = 0;
	uint8_t data_byte1 = 0; 
	uint8_t data_byte2 = 1; 
    /* suspend all recalibrations */
	ret = write_mem(acquisition_config_address + 6, 1, &data_byte1);
	ret = write_mem(acquisition_config_address + 7, 1, &data_byte2);
    /* don't recalibrate, flag set to 1 */
	ts->Recal_flag = 1;

	//ret = write_mem(command_processor_address + CALIBRATE_OFFSET, 1, &data_byte2);
...
/* delete some lines the multi_touch_mode and is_multi_touch will not use anymore*/
    ...
    /* delete some lines which is not needed */
    /* the flag of the first or second dot is pressed */
    u8 report_type;
    uint8_t data = 1;
	...
	switch (obj)
	{
		...
		case TOUCH_MULTITOUCHSCREEN_T9:
		    ...
		    point_index,((1 << 5) & status) ? "yes":"no", ts->touch_x, ts->touch_y);
            /* record the value */
    		ts->finger_data[report_type].x = ts->touch_x;	
    		ts->finger_data[report_type].y = ts->touch_y;
...
/* delete the goto function so if the touch is in the key area this will not be run */
                    ...
                    {   
                        if(1 == point_index)
                        {
                            ...
                            /* delete some lines that is not needed */
                        }
                        else
                        {
                        ...
                        /* press at the first time */
            			if(!(ts->finger_pressed & BIT(report_type)))
            			{
                            /* finger number increase */
            				ts->finger_count++;
            				ts->finger_pressed |= BIT(report_type);
                            /* need to recalibrate */
            				if (ts->Recal_flag == 0) 
            				{
                                /* record the first coordinate */
            					ts->pre_data[report_type].x = ts->finger_data[report_type].x;
            					ts->pre_data[report_type].y= ts->finger_data[report_type].y;
                                /* if having 2 finger */						
            			...
            			if (ts->finger_pressed & BIT(report_type)) 
            			{
            				...
            				/* if there is no finger count and in need re-calibration flag is set, then check the time and position */
            					...
            					if (ts->finger_count == 0 && 
            						...
            						((jiffies > ts->timestamp + 60 * HZ) ||
            						/* this condition need tuning for different case */
            						/* the first point and y > 400 and delta(x) > 150 */
            						(report_type == 0 && ts->finger_data[report_type].y > 420 && ((ts->finger_data[report_type].x - ts->pre_data[report_type].x) > 310))))
                                {
            						/* stop calibrate */
        ...
        case PROCG_GRIPFACESUPPRESSION_T20:         
            status = *(touch_msg + 1);
            if(0 != (status & 0x01)&&(ts->Recal_flag == 0))
            {
                check_chip_calibration();
            }
...
/* delete some lines. */
	...
	* is known. */
	...
	debug_diagnostic_address = get_object_address(DEBUG_DIAGNOSTIC_T37, 0);
 	acquisition_config_address = get_object_address(GEN_ACQUISITIONCONFIG_T8, 0);
