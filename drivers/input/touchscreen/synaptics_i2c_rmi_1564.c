...
#define SENSITIVE 8
/* deleted for reduce the power consumption of touch */
...
/* use this data for 8820 */
#define TS_KEY_Y_MAX_8820 138
...
#define Y_START_8820    (TS_Y_MAX-TS_KEY_Y_MAX_8820)
...
/* the same as that in the proc_misc.c */
...
/* read some information of the touch */
static int tp_read_input_name(struct synaptics_rmi4 *ts)
{
	int ret;
...
static int tp_read_proc(char *page,  char **start, off_t off, int count, int *eof, void *data)
{
	int len;

	len = snprintf(page, PAGE_SIZE, "TP_TYPE: %s\n"
		"Manufacturer ID: %x\n"
	...
	if (ts->hasF11) {
		for (i = 0; i < ts->f11.points_supported; ++i) {
          if(ts->is_support_multi_touch)
          {
            ...
            if(machine_is_msm7x30_u8820())
            {
			    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, ts->f11_max_y - TS_KEY_Y_MAX_8820, 0, 0);
            }
            else
            {
          ...
          else
          {
            ...
            if(machine_is_msm7x30_u8820())
            {
                input_set_abs_params(ts->input_dev, ABS_Y, 0, ts->f11_max_y - TS_KEY_Y_MAX_8820, 0, 0);
            }
            else
            {
...
/* Set the key value according to productions, the default value is U8800 */
    	if(machine_is_msm7x30_u8820())
		{
            ...
            touch_extra_key_region.extra_touch_region.touch_y_start= Y_START_8820;
            touch_extra_key_region.extra_key[0].center_y = TS_Y_MAX-TS_KEY_Y_MAX_8820/2+29;
            touch_extra_key_region.extra_key[1].center_y = TS_Y_MAX-TS_KEY_Y_MAX_8820/2+29;
            touch_extra_key_region.extra_key[2].center_y = TS_Y_MAX-TS_KEY_Y_MAX_8820/2+29;
            touch_extra_key_region.extra_key[3].center_y = TS_Y_MAX-TS_KEY_Y_MAX_8820/2+29;
		}
		else
		{
    ...
    gpio_config = GPIO_CFG(GPIO_TOUCH_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);
...
/*update firmware function come from 7x25*/
struct RMI4_FDT{
